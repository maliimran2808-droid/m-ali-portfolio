...
export default function SkillCards() {
  ...
  useEffect(() => {
    ...
    const finalX = isTablet ? [-240, 0, 240] : [-345, 0, 345];
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        trigger: section,
        start: "top 8%",
        end: "+=350%", // Increased scroll distance to stretch out the animation
        pin: true,
        scrub: 3,      // Increased scrub for more momentum smoothing
        anticipatePin: 1,
      },
    });
    ...
    cardEls.forEach((card, i) => {
      tl.to(card, { x: finalX[i], duration: 1, ease: "power1.inOut" }, 0);
    });

    cardInners.forEach((inner, i) => {
      tl.to(inner, { rotateZ: tiltAngles[i], duration: 1, ease: "power1.inOut" }, 0);
    });

    // ── SLOWER, MORE BALANCED FLIP ──
    cardInners.forEach((inner, i) => {
      tl.to(
        inner,
        {
          rotateY: 180,
          rotateZ: 0,
          duration: 1.2,        // Takes much longer relative to the scroll distance
          ease: "power2.inOut", // Smoother turn curve
        },
        1.1 + i * 0.2           // Tightly overlaps the cards
      );
    });
          ...
          {cards.map((card) => (
            ...
            >
              ...
              <div className="overflow-hidden">
                <h3
                  className="slide-up-text font-[family-name:var(--font-montserrat)] font-[600] uppercase tracking-normal text-white"
                  style={{ fontSize: "22px" }}
                >
                  {card.title}
                </h3>
              ...
              <div className="flex flex-col mt-4">
                {card.skills.map((skill, i) => (
                  <div key={i}>
                    ...
                    {i < card.skills.length - 1 && <div className="h-px w-full bg-white/10" />}
                  </div>
                ))}
              </div>
              ...
              <div className="flex justify-end mt-4 overflow-hidden">
                <span
                  className="slide-up-text font-[family-name:var(--font-montserrat)] font-[600]"
                  style={{ fontSize: "64px", color: "#f45100" }}
                >
                  {card.number}
                </span>
              </div>
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-0">
                      {card.skills.map((skill, i) => (
                        <div key={i}>
                          <p
                            className="font-[family-name:var(--font-montserrat)] py-3 text-xs font-medium uppercase tracking-widest text-white/60 md:text-sm"
